/** base object method introduction and interception **/
function abbr_string(s,len){
	if(s && s.length>len){
...
function containsArray(item,arr){
	if(arr){
...
function highlightTableRows(tableId) {
    highlightTableRowsWithCheck(tableId, true);
}

function highlightTableRowsWithCheck(tableId, addOnClickEvent) {
...
function getColumnCount(row){
    ...
    for(var i=0;i<row.childNodes.length;i++){
		var col = row.childNodes[i];
			if(col.tagName!="TD")
				continue;
        columnCount += col.colSpan;
	}
	return columnCount;
}

function selectAllCheckBox(checkboxName,checked){
...
/* Toggle checkboxes.
 * checkBoxObj: All checkboxes with same style class name of this will be toggled. Note: there could be multiple class names on one tag.
 * containerObj: Checkboxes group container. Default is body. Use this to improve performance.
 */
function toggleCheckboxes(checkBoxObj,containerObj){
	if(containerObj==null){
		containerObj=document.body;
	}
	$("."+checkBoxObj.className).each(function(i){
		this.checked=checkBoxObj.checked;
	});
}

/* Delayed executing action.
 * execStr: to eval script string.
 * delayTime: delay time.
 */
function DelayedAction(execStr,delayTime){
	var timeId;
	this.count=0;
	this.doAction=function(){
		this.clear();
		timeId=window.setTimeout(execStr,delayTime);	
	}
	this.clear=function(){
...
/* TabPanes.
 * tabPanesId: tabPanes's id in HTML page.
 */
function TabPanes(tabPanesId){

	var tabPanesObj=$("#"+tabPanesId);

	function switchTabPane(tabObj){

		var selectedIndex=0;
		$(".tab",tabPanesObj).each(function (i){
			var obj=$(this);
			if(this==tabObj){
		...
		$(".tabPane",tabPanesObj).each(function (i){
			var obj=$(this);
			if(i==selectedIndex){
				obj.show();
			}else{
	...
	function showFirstTabPane(){
		var tabs=$(".tab",tabPanesObj);
		//alert("tabs:"+tabs);
		if(tabs!=null){
			switchTabPane(tabs[0]);
		}
	}
...
function checkOneNotes(notesObj, caption, notesMaxLen){
	if(notesObj!=null){
		var notes=notesObj.value;
		notes=fixNewlinesOfTextarea(notes);

		var notesLen=notes.length;
		var result=notesLen<=notesMaxLen;
		if(!result){
			alert(caption+" can not be greater than "+notesMaxLen+" characters (now there're "+notesLen+" characters).");
		}
		return result;
	}else{
		return true;
...
function checkNotes(formObj, notesMaxLen){
	return checkOneNotes(formObj.notes,'Notes',notesMaxLen) && checkOneNotes(formObj.privateNotes,'Private Notes',notesMaxLen);
}
//FireFox2 need this(IE6 doesn't)
function syncBgColorWithOption(selectObj){
	selectObj.className=selectObj.options[selectObj.selectedIndex].className;
}

function switchBlock(obj,isVisible){
	var display=isVisible?"":"none";
	var disabled=isVisible?false:true;

	obj.style.display=display;
	obj.disabled=disabled;

	var childNodes=obj.getElementsByTagName('*');
...
function disableCCAutoComplete(){
	var inputNames=["creditCard.number","newPayment.creditCard.number","ccNumber"];
