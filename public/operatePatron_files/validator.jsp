...
function validateEmailFormat(form) {
    ...
    oEmail = eval('new ' + formName.value + '_emailformat()');
    ...
    for (x in oEmail) {
        ...
        if ((field.type == 'hidden' || 
            ...
            (field.value.length > 0) &&
            field.disabled == false) {
            if (!checkEmailFormat(field.value)) {
                if (i == 0) {
                    focusField = field;
                }
                fields[i++] = oEmail[x][1];
...
function testReg(regExStr,value){
	var p=new RegExp(regExStr);
	return p.test(value);
}
function checkEmailFormat(email){
	...
	if(email_array.length != 2) return false;
	if ((email_array[0].length < 1) || (email_array[1].length < 1))	return false;
   	var local_array =email_array[0].split(".");
   	for (var i = 0; i < local_array.length; i++) {
   		if (!testReg("^(([A-Za-z0-9!#$%&'*+/=?^_`{|}~-][A-Za-z0-9!#$%&'*+/=?^_`{|}~.-]{0,63})|(\"[^(\\|\")]{0,62}\"))$", local_array[i])) {
   			return false;
   		}
   	}
   	if (!testReg("^[?[0-9.]+]?$", email_array[1])) { // Check if domain is IP. If not, it should be valid domain name
   		var domain_array = email_array[1].split(".");
   		if (domain_array.length < 2) {
   			return false; // Not enough parts to domain
   		}
   		for (var i = 0; i < domain_array.length; i++) {
   			if (!testReg("^(([A-Za-z0-9][A-Za-z0-9-]{0,61}[A-Za-z0-9])|([A-Za-z0-9]+))$", domain_array[i])) {
   				return false;
   			}
   		}
   	}
